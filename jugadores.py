...
# Draw the player information in the top right corner
def draw_player_info():
    # Draw the player info box
    player_info_rect = pygame.Rect(600, 20, 240, 130)
    pygame.draw.rect(screen, BLUE, player_info_rect)
    
    # Draw player name
    font = pygame.font.SysFont("Arial", 16)
    player_name_text = font.render(f"Jugador: {player['name']}", True, WHITE)
    screen.blit(player_name_text, (player_info_rect.x + 10, player_info_rect.y + 10))

    # Draw the chip counts
    y_offset = 40
    for value, count in player['chips'].items():
        chip_text = font.render(f"Fichas {value}: {count}", True, WHITE)
        screen.blit(chip_text, (player_info_rect.x + 10, player_info_rect.y + y_offset))
        y_offset += 25
...
def draw_custom_buttons():
    for button in custom_buttons:
        ...
        pygame.draw.rect(screen, WHITE, rect, 1)  # Draw a white border around the button
...
def app_draw():
    ...
    # Draw player info box in the top right corner
    draw_player_info()
    
    # Draw the roulette wheel
    center = {"x": 300, "y": 250}
    for i, casella in enumerate(rule): 
        start_angle = (360 / 37) * i + angle  # starting angle to draw segment
        end_angle = (360 / 37) * (i + 1) + angle  # ending angle

        # Points for the larger circle
        p0 = utils.point_on_circle(center, 100, start_angle)
        p1 = utils.point_on_circle(center, 150, start_angle)
        p2 = utils.point_on_circle(center, 150, end_angle)
        p3 = utils.point_on_circle(center, 100, end_angle)

        # Colors for roulette
        color = GREEN if i == 0 else RED if i % 2 == 1 else BLACK

        # Coordinates for drawing the arrow
        coords = [
            (290, 90),
            (310, 90),
            (300, 115)
        ...
        pygame.draw.polygon(screen, color, [(int(p0["x"]), int(p0["y"])), 
                                           (int(p1["x"]), int(p1["y"])), 
                                           (int(p2["x"]), int(p2["y"])), 
                                           (int(p3["x"]), int(p3["y"]))])

        # Place the number in the center of each roulette segment
        centre_quesito = utils.point_on_circle(center, 140, (start_angle + end_angle) / 2)
